...
function renderLlmSection(llm) {
  ...
  const execSummary = scored.length === 0
    ? `**SCAN DID NOT COMPLETE.** All ${s.num_attacks} attacks in this run errored before a response could be scored (see the error column in the findings table below) - this is not evidence of a clean or passing result. Re-run \`sentinel-scan\` against a reachable endpoint before treating this section as compliance evidence.`
    : `The target system was tested against ${s.num_attacks} adversarial prompts spanning ${categoriesCovered.length} OWASP LLM Top 10 categories (sentinel-scan-cli v${s.version}). ` +
